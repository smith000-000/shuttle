...
- explicit `codex_app_server` selection now uses a native Codex App Server client over stdio JSON-RPC. Shuttle keeps a long-lived app-server process alive for the runtime session, reuses native thread bindings per task across continuation turns, lets ordinary app-server turns execute tools natively inside that runtime thread, and routes manual `/compact` through the bound native thread when one exists instead of synthesizing a Shuttle-owned summary on a fresh turn
...
This means the selection seam is live and authoritative, but product ownership still stays inside Shuttle. The current `codex_sdk` implementation remains the primary CLI-backed bridge. `codex_app_server` now exists as a separate runtime that uses the real app-server transport for turn handling and native tool execution while Shuttle still owns session/task state, approvals, transcript mutation, and the persistent user-shell continuity surface. Native compaction is now treated as part of the runtime thread contract: if the app-server thread is gone, Shuttle surfaces that loss explicitly instead of silently recovering onto a fresh thread for `/compact`. Hybrid builtin-to-secondary delegation is intentionally out of scope for this model and is tracked in [P5](P5.md).
