...
- Active: checklist progress is still too controller-driven and opaque. After an interrupted planned command (`F3` -> `Ctrl+C` -> `Ctrl+G`), continuation can desync plan progression because the controller advances the plan before the agent has rechecked actual shell state.
- Next: move checklist reconciliation to every agent turn. When an active checklist exists, the runtime prompt should require an explicit status check and the response should update per-step status instead of relying on optimistic controller advancement.
- Next: perform an architecture review of the recent shell-tracking, transition-settlement, pane-recovery, and handoff changes. The review should call out DRY/SRP/SOLID violations, identify duplicated fallback logic across controller/shell/TUI, and propose decomposition seams before more behavior is layered onto this path.
...
- Active checklists must update from explicit agent status reconciliation on continuation turns instead of controller-side blind advancement after command or patch completion.
