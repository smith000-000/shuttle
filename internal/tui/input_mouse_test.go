...
func TestHelpViewContainsSlashCommandsAndShortcuts(t *testing.T) {
	...
	for _, fragment := range []string{"/help", "/approvals", "/approvals dangerous", "/coder", "/new", "/compact", "/onboard", "/provider", "/model", "/quit", "Ctrl+]", "F2", "Shift-drag", "Ctrl+Shift+C / Ctrl+Shift+V", "KEYS> Enter", "KEYS> Ctrl+Y", "KEYS> Ctrl+J"} {
