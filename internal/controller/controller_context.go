...
func estimateContextUsage(input AgentInput) ContextWindowUsage {
	...
	appendUsageSection(&builder, input.Session.LocalWorkingDirectory)
	appendUsageSection(&builder, input.Session.LocalHomeDirectory)
	appendUsageSection(&builder, input.Session.LocalUsername)
	appendUsageSection(&builder, input.Session.LocalHostname)
