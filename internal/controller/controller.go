...
func continueAfterPatchApplyPrompt(target PatchTarget, label string) string {
	location := "the local workspace"
	if target == PatchTargetRemoteShell {
		location = "the remote shell target"
		if strings.TrimSpace(label) != "" {
			location += " (" + strings.TrimSpace(label) + ")"
		}
	}
	return "The previously approved or proposed patch was applied to " + location + ". First summarize the file changes briefly. If there is an active plan, continue it from the current step. If there is no active plan and the user's request is satisfied by the applied patch, stop after reporting the outcome. Do not propose extra verification or follow-up edits unless the user explicitly asked for them or the patch result/transcript shows unresolved work. If risky, request approval."
}

func continueAfterPatchFailurePrompt(target PatchTarget, label string, errText string, repairExhausted bool) string {
	location := "the local workspace"
	if target == PatchTargetRemoteShell {
		location = "the remote shell target"
		if strings.TrimSpace(label) != "" {
	...
	if repairExhausted {
		return "The previously proposed or approved patch failed again on " + location + ". Do not emit another corrected patch. Briefly summarize the repeated failure, then either propose a safer next step or tell the user what manual intervention is needed. For remote targets, a normal shell command approval is acceptable if it is the safest way forward."
	}
	if shouldReinspectAfterPatchFailure(target, errText) {
		return "The previously proposed or approved patch did not apply cleanly on " + location + " because the patch context did not match the current file contents. Do not emit another patch yet. First summarize the conflict briefly, then propose exactly one safe read-only shell command to inspect the relevant file or anchor text on the active target so the next turn can build a corrected patch from fresh content. Do not propose a write command, apply_patch, git apply, patch, or a heredoc patch tool in this recovery step."
	}
	return "The previously proposed or approved patch did not apply cleanly on " + location + ". First summarize the patch failure briefly. If the task still needs a file change, propose exactly one corrected patch as proposal_kind:\"patch\" with a valid unified diff. Treat proposal_patch as a strict tool payload, not prose. proposal_patch must start immediately with the first diff header, contain no preamble, no code fence, no commentary, exact hunk headers and counts, and a complete patch body with no truncation. Set the patch target explicitly. Do not emit a shell command that invokes apply_patch, git apply, patch, or any heredoc-based patch tool. If the next step is risky, request approval. If the failure already resolves the task or the user should intervene manually, answer briefly."
}

func shouldReinspectAfterPatchFailure(target PatchTarget, errText string) bool {
	if target != PatchTargetRemoteShell {
		return false
	}
	errText = strings.ToLower(strings.TrimSpace(errText))
	return strings.Contains(errText, "conflict: fragment line does not match src line")
}
...
const inspectContextCommandLabel = "inspect current shell context"
...
type LocalController struct {
	...
	patchInitErr error
	remoteCaps   *remoteCapabilityStore
